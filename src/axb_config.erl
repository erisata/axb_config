...
decode_value(term, Value) when is_list(Value) ->    % TODO: Handle `term' variables from the itests (when passed explicitly, not from a file) without parsing them, not terms must be passed as strings in itests (e.g. `"[]"' instead of `[]').
